...
af<-function(qwer){
  ...
  #set.seed(778)
  if(ONESTAGE){simu_data_2=data.frame()}
  else {simu_data=data.frame(stage=rep.int(2,SSIZE))
  for (h in node.ordering(ran)) {
    a1=0.8
    a2=0.7
    b1=1
  ...
  #set.seed(779)
  if(ONESTAGE) {simu_data_3=data.frame()}
  else {simu_data=data.frame(stage=rep.int(3,SSIZE))
  for (h in node.ordering(ran)) {
    a1=0.8
    a2=0.7
    b1=1
  ...
  if(discretized==T){d_simu_data<-as.data.frame(apply(continuous_simu_data, 2, discretize_kmeans))}
  else{d_simu_data<-as.data.frame(continuous_simu_data)}
  ...
  cu_su_result=data.frame(
    num_true_arcs=nrow(arcs(ran)),
    num_all_arcs=nrow(half_bidi_bn_arcs),
    num_arcs=nrow(arcs.compare(half_bidi_bn_arcs,arcs(ran))),
    num_dire=nrow(direction.compare(pure_direct_bn_arcs,arcs(ran))),
    num_xxx_dire=nrow(direction.compare(pure_direct_A1B2_arcs,arcs(ran)))
  )
  ...
  cu_ch_result=data.frame(
    num_true_arcs=nrow(arcs(ran)),
    num_all_arcs=nrow(half_bidi_bn_arcs),
    num_arcs=nrow(arcs.compare(half_bidi_bn_arcs,arcs(ran))),
    num_dire=nrow(direction.compare(pure_direct_bn_arcs,arcs(ran))),
    num_xxx_dire=nrow(direction.compare(pure_direct_A1B2_arcs,arcs(ran))),
    num_ch_to_su_dire=nrow(direction.compare(pure_direct_A1B2_arcs,su_arcs))
    
  )
  #cu_ch_results<-rbind(cu_ch_results,cu_ch_result)
  return(list(o=only_cu_result,s=cu_su_result,h=cu_ch_result))
}

parameters=list(n_nodes=50,
                NOISE=1, #random niose
                SNOSIE=0.2 ,#noise of h0 in each stage
                SSIZE=1000 ,#each stage size, if only one stage, it is larger
                ONESTAGE = F,
                ...
                LATENT=F ,#whether consider latent variables
                LAT_p=0.9 ,#after remove some variables, how many remains
                discretized=T ,#whether to discretize simulated data
                test_method = "mi-g-sh", #CI test method,depending on whether discretizing data
                #above
                #need
                #change
                ...
                n_iter=500,
                n_clusters=7,
                rds_file="./50/pc_limitedmaxsz_multi_nolat.rds"
)
...
library(snow)
library(rlecuyer)
cl=makeSOCKcluster(parameters$n_clusters)
clusterSetupRNG(cl)
r_list=parApply(cl,X=matrix(nrow = parameters$n_iter,ncol =100),1,af)
#saveRDS(r_list,parameters$rds_file)
stopCluster(cl)
