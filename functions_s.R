...
if(F){
#correct arcs and directions for only current
correct_arcs_direction<-function(true_graph,learned_graph){

  #clean the repilicate arcs, remain the first one
  learned<-data.frame()
c_list<-c()
for (geneA in names(learned_graph$nodes)) {
  #print(geneA)
  c_list<-append(c_list,geneA)
  for (geneB in nbr(learned_graph,geneA)) {
    if(is.element(geneB,c_list)) next
    else {learned<-rbind(learned,data.frame(geneA,geneB,stringsAsFactors = F))
    }    
  }
}
learned<-substr(as.matrix(learned),1,1)

truth<-arcs(true_graph)
#sum correct arcs
x=apply(learned[,],1,function(x){
  #print(x)
  a=matrix(truth[truth[,1]==x[[1]],],ncol = 2,byrow = F)
...
correct_arcs=sum(x)
#sum correct arcs(minus bi direct)
y=apply(substr(arcs(learned_graph),1,1),1,function(x){
  #print(x)
  a=matrix(truth[truth[,1]==x[[1]],],ncol = 2,byrow = F)
  b=matrix(truth[truth[,2]==x[[1]],],ncol = 2,byrow = F)
  if(sum(a[,2]==x[[2]])==1) return(1)
...
correct_directions=sum(y)-(nrow(arcs(learned_graph))-nrow(learned))
return(c(correct_arcs,correct_directions))
}
#differ from correct_arcs_direction,input is arcs
compare_arcs_matrix<-function(learned,truth){
  #sum correct arcs
  x=apply(learned[,],1,function(x){
    #print(x)
    a=matrix(truth[truth[,1]==x[[1]],],ncol = 2,byrow = F)
  ...
  correct_arcs=sum(x)-sum_bi_directions(learned)-sum_same_directions(learned)
  #sum correct arcs(minus bi direct)
  y=apply(learned,1,function(x){
    #print(x)
    a=matrix(truth[truth[,1]==x[[1]],],ncol = 2,byrow = F)
  ...
  correct_directions=sum(y)-sum_bi_directions(learned)-sum_same_directions(learned)
  return(c(correct_arcs,correct_directions))
}

#for current+subsequent
#conver to true graph based on bn: A1->B2 means A->B, B2->A1 means B->A
learned_graph=bn.pc.cu.su
bn_graph_arcs=data.frame()
x=apply(arcs(learned_graph), 1, function(x){
  x=strsplit(x,"")
  if(x$from[1]!=x$to[1]){
...
#convert based on XXX framework
xxx_graph_arcs=data.frame()
x=apply(arcs(learned_graph), 1, function(x){
  x=strsplit(x,"")
  if(x$from[1]!=x$to[1]){
